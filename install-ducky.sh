#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
#  sovereign — personal install for Ducky
#  delivered by Marcus
#
#  bash install-ducky.sh
# ═══════════════════════════════════════════════════════════════════════════════
set -e
...
# ── THE TOKEN ─────────────────────────────────────────────────────────────────
_T="sovsZegAOBPVXFMCIoZZiHbTYKeCTBoRDsCyi4IaxWJ"
...
echo "  │   RECIPIENT:        DUCKY                                  │"
echo "  │   DOC REF:          AXM-$(date +%Y)-$(date +%m%d)-DUCKY               │"
...
# ── SAVE TOKEN ────────────────────────────────────────────────────────────────
echo "$DUCKY_TOKEN" > "$HOME/.axis-token-ducky"
chmod 600 "$HOME/.axis-token-ducky"
echo "$DUCKY_TOKEN" > "$HOME/.axis-token"
chmod 600 "$HOME/.axis-token"
echo -e "  ${LIME}✓${RST}  token saved (ducky + default)"

# ── NOTIFY ────────────────────────────────────────────────────────────────────
curl -s -X POST "https://axismundi.fun/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $DUCKY_TOKEN" \
    -d "{\"model\":\"dolphin-mistral:latest\",\"messages\":[{\"role\":\"user\",\"content\":\"sovereign install-ducky completed on $(hostname) at $(date)\"}],\"max_tokens\":1}" \
    &>/dev/null &
...
echo "  ┌─────────────────────────────────────────────────────────────┐"
echo "  │                                                             │"
echo "  │      ${LIME}sovereign is ready, Ducky.${WHITE_FG}                           │"
echo "  │                                                             │"
