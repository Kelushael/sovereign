#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
#  sovereign — personal install for Kyree
#  delivered by Marcus
#
#  bash install-kyree.sh
# ═══════════════════════════════════════════════════════════════════════════════
set -e
...
# black-on-black = the redaction bar
BLACK_BG='\033[40m'; BLACK_FG='\033[30m'
WHITE_FG='\033[97m'; DIM='\033[2m'

# ── THE TOKEN (Marcus fills this in before delivering) ────────────────────────
...
echo "  │   RECIPIENT:        KYREE                                  │"
echo "  │   DOC REF:          AXM-$(date +%Y)-$(date +%m%d)-KYREE               │"
...
echo -e "  ${GRAY}Name:        Kyree${RST}"
echo -e "  ${GRAY}Stack:       Axis Mundi / Sovereign Terminal${RST}"
echo -e "  ${GRAY}Access:      Full stack — your iron, your model, your rules${RST}"
echo -e "  ${GRAY}Model host:  axismundi.fun (VPS, zero corporate filter)${RST}"
echo ""
sleep 0.3
...
if [ "$CLEARANCE" = "yhwh" ]; then
    ...
    # Reveal the token — with a dramatic typewriter effect
    echo -e "${BOLD}${WHITE_FG}"
    echo "  ┌─────────────────────────────────────────────────────────────┐"
    ...
    echo -e "  ${GRAY}contact Marcus.${RST}\n"
    exit 1
fi
...
# ── SAVE TOKEN ────────────────────────────────────────────────────────────────
echo "$KYREE_TOKEN" > "$HOME/.axis-token-kyree"
chmod 600 "$HOME/.axis-token-kyree"
echo "$KYREE_TOKEN" > "$HOME/.axis-token"
chmod 600 "$HOME/.axis-token"
echo -e "  ${LIME}✓${RST}  token saved (kyree + default)"

# ── NOTIFY ────────────────────────────────────────────────────────────────────
# fire-and-forget ping so Marcus knows Kyree is live
curl -s -X POST "https://axismundi.fun/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $KYREE_TOKEN" \
    -d "{\"model\":\"dolphin-mistral:latest\",\"messages\":[{\"role\":\"user\",\"content\":\"sovereign install-kyree completed on $(hostname) at $(date)\"}],\"max_tokens\":1}" \
    &>/dev/null &
...
echo "  ┌─────────────────────────────────────────────────────────────┐"
echo "  │                                                             │"
echo "  │      ${LIME}sovereign is ready, Kyree.${WHITE_FG}                           │"
echo "  │                                                             │"
echo "  │   ${GRAY}sovereign --kyree${WHITE_FG}   →  open your stack                │"
echo "  │   ${GRAY}sovereign${WHITE_FG}            →  also works                    │"
